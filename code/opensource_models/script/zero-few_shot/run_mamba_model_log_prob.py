            ...
            for index, impression in tqdm(df.iterrows(), desc="Starting"):
                ...
                for i, condition in enumerate(conditions):
                    ...
                    if "yes" in answer.lower():
                        ...
                        yes_prob = np.max([outputs.scores[-1][:,4374].numpy()[0], outputs.scores[-1][:,9820].numpy()[0], outputs.scores[-1][:,24239].numpy()[0]])
                        no_prob = np.max([outputs.scores[-1][:,2302].numpy()[0], outputs.scores[-1][:,2369].numpy()[0], outputs.scores[-1][:,7716].numpy()[0]])
                        if yes_prob > no_prob:
                            label = "Yes"
                        else: label = "No"
